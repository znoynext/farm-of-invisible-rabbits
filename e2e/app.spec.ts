...
async function openRadar(page: Page) {
  await page.goto("/");
  await page
    .getByRole("button", { name: "Посмотреть текущую оценку" })
    .click();
  await expect(
...
test("показывает Intro только при первом посещении", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { level: 2, name: "Как это работает" }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", {
      level: 2,
  ...
  await expect(page.getByText("Обзор", { exact: true })).toBeVisible();
  await expect(page.getByText("Сигналы", { exact: true })).toBeVisible();
  await expect(page.getByText("Модель", { exact: true })).toBeVisible();
  await expect(page.getByText("AI Worklog", { exact: true })).toBeVisible();
...
test("Intro сохраняет мобильную композицию без horizontal overflow", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("button", { name: "Посмотреть текущую оценку" }),
  ).toBeVisible();
  await expect.poll(() =>
...
test("ориентирует пользователя по трём следующим действиям", async ({ page }) => {
  await openRadar(page);

  await expect(
    page.getByRole("heading", { level: 2, name: "Что можно сделать дальше" }),
  ).toBeVisible();
  await expect(page.getByRole("link", { name: "Открыть сигналы" })).toBeVisible();
  await expect(page.getByRole("link", { name: "Проверить гипотезу" })).toBeVisible();
  await expect(page.getByRole("link", { name: "Открыть модель" })).toBeVisible();

  await page.getByRole("link", { name: "Проверить гипотезу" }).click();
  await expect(page).toHaveURL(/#scenario-lab$/);
  await expect(
    page.getByRole("heading", { level: 2, name: "Проверить гипотезу" }),
  ).toBeFocused();

  await page.getByRole("link", { name: "Открыть сигналы" }).click();
  await expect(page).toHaveURL(/#signals$/);
  await expect(
    page.getByRole("heading", { level: 1, name: "Сигналы" }),
  ).toBeVisible();

  await page.getByRole("link", { exact: true, name: "Обзор" }).click();
  await expect(page.getByRole("link", { name: "Открыть модель" })).toBeVisible();
  await page.getByRole("link", { name: "Открыть модель" }).click();
...
test("открывает app shell и переключает раздел", async ({ page }) => {
  ...
  await page.getByRole("link", { exact: true, name: "Сигналы" }).click();
...
test("показывает доступный публичный AI Worklog без приватных данных", async ({ page }) => {
  await openRadar(page);
  await page.getByRole("link", { exact: true, name: "AI Worklog" }).click();

  await expect(page).toHaveURL(/#ai-worklog$/);
  await expect(
    page.getByRole("heading", { level: 1, name: "AI Worklog" }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", {
      level: 2,
  ...
  await expect(page.getByText("Как был сформулирован стартовый запрос к AI.")).toBeVisible();
  await expect(
    page.getByText("Какие проблемы нашли проверки и внешние аудиты и как их исправили."),
  ).toBeVisible();
...
test("проверяет observation-only гипотезу и сохраняет её только после применения", async ({ page }) => {
  ...
  await expect(recommendations).toContainText("Для временного сценария");
...
test("управляет сигналами и согласованно пересчитывает весь продукт", async ({
  ...
  await expect(page.getByText(/Сначала добавьте хотя бы одно наблюдение/)).toBeVisible();
...
test("подтверждает destructive Restore без скрытой потери scenario и model settings", async ({
  ...
  await page.getByRole("link", { exact: true, name: "Модель" }).click();
