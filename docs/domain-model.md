...
`buildRecommendations` возвращает не больше трёх готовых рекомендаций. Каждая содержит отдельные `action` и `reason`, чтобы UI не восстанавливал объяснение из порогов или строк. При пустом наборе наблюдений domain result предлагает начать с первого наблюдения.
